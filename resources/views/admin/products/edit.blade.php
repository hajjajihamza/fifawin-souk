...
@section('page-title', 'Édition : ' . $product->name)

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Référence -->
                <div class="space-y-2">
                    <label for="reference" class="text-sm font-semibold text-gray-700">Référence</label>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                        @if($product->image)
                            <img src="{{ asset('storage/' . $product->image) }}" class="w-16 h-16 rounded-lg object-cover border border-gray-200 shrink-0" alt="{{ $product->name }}">
                        @endif
                        <input type="file" name="image" id="image"
                            class="w-full flex-1 px-4 py-1.5 border border-gray-300 rounded-lg file:mr-4 file:py-1 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 transition-all @error('image') border-red-500 @enderror">
                    </div>
            ...
            <div class="space-y-2">
                ...
                <textarea name="note" id="note" rows="4"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all @error('note') border-red-500 @enderror">{{ old('note', $product->note) }}</textarea>
